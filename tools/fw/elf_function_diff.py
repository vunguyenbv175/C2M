...
Raw byte hashes intentionally over-report semantic changes when addresses move;
use this as a candidate generator, then inspect/canonicalize relocations and
PC-relative references before drawing conclusions.
"""
...
def sections(path: Path):
    ...
    for line in txt.splitlines():
        ...
        if m:
            idx = int(m.group(1))
            out[idx] = {"name": m.group(2), "type": m.group(3), "addr": int(m.group(4), 16), "offset": int(m.group(5), 16), "size": int(m.group(6), 16)}
    return out
...
def funcs(path: Path):
    ...
    for line in txt.splitlines():
        cols = line.split()
        if len(cols) < 8 or not cols[0].endswith(":") or cols[3] != "FUNC":
            continue
        try:
            value = int(cols[1], 16); size = int(cols[2]); ndx = int(cols[6])
        except ValueError:
            continue
        if size <= 0 or ndx not in secs:
            continue
        sec = secs[ndx]; rel = value - sec["addr"]; off = sec["offset"] + rel
        if rel < 0 or off < 0 or off + size > len(data):
            continue
        blob = data[off:off + size]; name = cols[7]
        item = {"value": value, "value_hex": hex(value), "size": size, "section": sec["name"], "file_offset": off, "file_offset_hex": hex(off), "sha256": hashlib.sha256(blob).hexdigest()}
        prev = out.get(name)
...
def main():
    ...
    args = ap.parse_args()
    a, b = funcs(args.left), funcs(args.right)
    common = sorted(set(a) & set(b)); changed = []; same = []
    for n in common:
